...
def styleHisto(histo, color, xtit, ytit, filled=False):
...
h_d.SetName("Data")
h_e = file_d.Get(histname)
h_m = file_d.Get(histname)
h_t = file_d.Get(histname)
styleHisto(h_d, ROOT.kYellow, "M_{ll} [GeV]", "Entries")
...
if file_e:
    ...
    h_e.Scale(lumi*xsec_e/sumw_e/3.5)
    h_e.SetName("electron")
    styleHisto(h_d, ROOT.kBlue, "M_{ll} [GeV]", "Entries")
    legend.AddEntry(h_e, "MC Z->ee", "f")
    stack.Add(h_e)
if file_m:
    h_m = file_m.Get(histname)
    h_m.Scale(lumi*xsec_m/sumw_m/3.5)
    h_m.SetName("muon")
    styleHisto(h_m, ROOT.kGreen, "M_{ll} [GeV]", "Entries")
    legend.AddEntry(h_e, "MC Z->mumu", "f")
    stack.Add(h_m)
if file_t:
    h_t = file_t.Get(histname)
    h_t.Scale(lumi*xsec_t/sumw_t/3.5)
    h_t.SetName('tau')
    styleHisto(h_t, ROOT.kRed, "M_{ll} [GeV]", "Entries")
    legend.AddEntry(h_e, "MC Z->tautau", "f")
    stack.Add(h_t)
...
h_d.Draw("same")
h_d.GetXaxis().SetRangeUser(79,102)
