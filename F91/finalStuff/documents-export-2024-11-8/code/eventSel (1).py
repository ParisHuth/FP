...
outfile = ROOT.TFile.Open("analysis_Muon_weight.root", "RECREATE")
...
for event in tree:
    if current_count >= max_count:
        break
    weight = event.mcWeight*event.scaleFactor_PILEUP*event.scaleFactor_ZVERTEX*event.scaleFactor_ELE*event.scaleFactor_MUON*event.scaleFactor_TRIGGER
    current_count +=1
...
canvas.SetLogy()
canvas.Draw()
canvas.SaveAs("Muon_weight_cutFlowHist.png")
...
invMass_cut.Draw()
c2.Draw()
c2.SaveAs("Muon_weight_invMass_cut.png")
